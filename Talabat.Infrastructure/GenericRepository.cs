...
namespace Talabat.Infrastructure
{
	public class GenericRepository<T> : IGenericRepository<T> where T : BaseEntity
	{
		...
		{
			return await ApplySpecifiactions(spec). AsNoTracking().ToListAsync();
		}

		public async Task<T?> GetWithSpecAsync(ISpecifications<T> spec)
		{
			return await ApplySpecifiactions(spec).FirstOrDefaultAsync();

		}


		private IQueryable<T> ApplySpecifiactions(ISpecifications<T> spec) 
		{
